...
class UniappOpenWebpackPlugin {
   ...
   */
  getMiniCli() {
    const home = process.env.WX_MINIPROGRAM_HOME || process.env.WX_MINI_HOME
    if (process.env.WX_MINIPROGRAM_HOME) {
  ...
  open() {
    ...
    if (!fs.pathExistsSync(this.minniCli)) {
      console.log(
        chalk.bgRed(
          '使用 UniappOpenWebpackPlugin 插件，请先按要求配置系统环境变量'
        )
      )
      return
    }
  ...
  apply(compiler) {
    compiler.hooks.done.tap('UniappOpenWebpackPlugin', (stats) => {
...
module.exports = UniappOpenWebpackPlugin
